AutoML<-function(DataFrame,Split_Value,Size,SMOTE){
  ...
  #Split File
  data=DataFrame
  split = sample.split(data$result, SplitRatio = Split_Value)
  ...
  if (SMOTE=='Y') {
    #str(training_set$result)
    prop.table(table(training_set$result))
    training_set<-SMOTE(result ~ ., training_set, perc.over = 100, perc.under=200)
    #prop.table(table(training_set$result))
  ...
  #Load file
  train_file <- training_set  #read.csv("train.csv", stringsAsFactors = FALSE)
  ...
  #Create the Multifolds based on the labels result
  cv.10.folds<-createMultiFolds(train_file_clean$result, k=10, times =10)
  #Create the train control 
  ctrl.1<-trainControl(method = "repeatedcv",number = 10, repeats = 10, index = cv.10.folds)
  #Create subset with only X Values
  traing_x<-subset(train_file_clean, select= -c(result))
  ...
  # Check sensibity and 
  summary(train_file_clean$result)/(Total_Records)
  ...
  ZeroR_Classifier<-ZeroR(train_file_clean$result~ ., data = train_file_clean)
  ...
  ZeroR_Test<-table( predict(ZeroR_Classifier,newdata=test_set),test_set$result )
  
  #===============OneR===================
  
  OneR_Classifier<-train(x = traing_x , y =training_set$result , method = "OneR", trControl = trainControl())
  OneR_Train<-summary(OneR_Classifier)
  #Cross Validation
  OneR_CV_Model<-train(x = traing_x , y =training_set$result , method = "OneR", trControl = ctrl.1)
  OneR_CV<-summary(OneR_CV_Model)
  OneR_Test<-table( predict(OneR_CV_Model,newdata=test_set),test_set$result )
  
  #===============J48===================
  
  J48_Classifier<-train(x = traing_x , y =training_set$result , method = "J48", trControl = trainControl())
  J48_Train<-summary(J48_Classifier)
  #Cross Validation
  J48_CV_Model<-train(x = traing_x , y =training_set$result , method = "J48", trControl = ctrl.1)
  J48_CV<-summary(J48_CV_Model)
  J48_Test<-table( predict(J48_CV_Model,newdata=test_set),test_set$result )
  ...
  #===============LMT===================
  LMT_Classifier<-train(x = traing_x , y =training_set$result , method = "LMT", trControl = trainControl())
  LMT_Train<-summary(LMT_Classifier)
  #Cross Validation
  LMT_CV_Model<-train(x = traing_x , y =training_set$result , method = "LMT", trControl = ctrl.1)
  LMT_CV<-summary(LMT_CV_Model)
  LMT_Test<-table( predict(LMT_CV_Model,newdata=test_set),test_set$result )
  ...
  #===============Logistic===================
  Logistic_Classifier<-Logistic(train_file_clean$result~ ., data = train_file_clean)
  ...
  Logistic_Test<-table( predict(Logistic_Classifier,newdata=test_set),test_set$result )
  ...
  BayesNet_Classifier<-BayesNet(train_file_clean$result~ ., data = train_file_clean)
  ...
  BayesNet_Test<-table( predict(BayesNet_Classifier,newdata=test_set),test_set$result )
  ...
  NaiveBayes_Classifier<-NaiveBayes(train_file_clean$result~ ., data = train_file_clean)
  ...
  NaiveBayes_Test<-table( predict(NaiveBayes_Classifier,newdata=test_set),test_set$result )
  ...
  #===============SMO===================
  SMO_Classifier<-SMO(train_file_clean$result~ ., data = train_file_clean)
  ...
  SMO_Test<-table( predict(SMO_Classifier,newdata=test_set),test_set$result )
  ...
  RandomForest_Test<-table( predict(RandomForest_Classifier,newdata=test_set),test_set$result )
  ...
  RandomTree_Classifier<-RandomTree(train_file_clean$result~ ., data = train_file_clean)
  ...
  RandomTree_Test<-table( predict(RandomTree_Classifier,newdata=test_set),test_set$result )
  ...
  REPTree_Classifier<-REPTree(train_file_clean$result~ ., data = train_file_clean)
  ...
  REPTree_Test<-table( predict(REPTree_Classifier,newdata=test_set),test_set$result )
  ...
  #===============DecisionStump===================
  DecisionStump_Classifier<-DecisionStump(train_file_clean$result~ ., data = train_file_clean)
  ...
  DecisionStump_Test<-table( predict(DecisionStump_Classifier,newdata=test_set),test_set$result )
  ...
  #===============PART===================
  PART_Classifier<-PART(train_file_clean$result~ ., data = train_file_clean)
  ...
  PART_Test<-table( predict(PART_Classifier,newdata=test_set),test_set$result )
  ...
  #===============IBk===================
  IBk_Classifier<-IBk(train_file_clean$result~ ., data = train_file_clean,control=Weka_control(K=1))
  ...
  IBk_Test<-table( predict(IBk_Classifier,newdata=test_set),test_set$result )
  ...
  MultilayerPerceptron_Classifier<-MultilayerPerceptron(train_file_clean$result~ ., data = train_file_clean)
  ...
  MultilayerPerceptron_Test<-table( predict(MultilayerPerceptron_Classifier,newdata=test_set),test_set$result )
  ...
  TN_Test<-summary(test_set$result)[2]#True Negative
  TP_Test<-summary(test_set$result)[1]#True Positive
  #Accuracy
...
Auto_ML_Bag_Bos_Ens<-function(Data,Size,Model1,Model2,Previous_Table){
  ...
  Bagging_Classifier<-Bagging(train_file_clean$result~ ., data = train_file_clean, control = Weka_control(W=Best_Model_1), na.action=NULL)
  ...
  #tryCatch(expr = {LogitBoost_Classifier<-LogitBoost(train_file_clean$result~ ., data = train_file_clean,control = Weka_control(W=Best_Model_1),na.action=NULL)
  #},finally = {LogitBoost_Classifier<-ZeroR(train_file_clean$result~ ., data = train_file_clean)})
  ...
  #===============AdaBoostM1===================
  AdaBoostM1_Classifier<-AdaBoostM1(train_file_clean$result~ ., data = train_file_clean,control = Weka_control(W=Best_Model_1), na.action=NULL)
  ...
  #===============Stacking===================
  Stacking_Classifier<-Stacking(train_file_clean$result~ ., data = train_file_clean,control = Weka_control(
    M=Best_Model_1,
